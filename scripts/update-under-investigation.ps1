# Import / refresh Under Investigation demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
#
# Multi-file zip convention (future imports):
#   When a source zip contains multiple WAV versions, designate ONE loop file for:
#     - demo.mp3 (128 kbps MP3 from the loop WAV)
#     - loop.zip ($9 tier — single loop WAV only)
#   Put ALL files from the source zip (or all remaining versions) into:
...
$trackDir = Join-Path $ProjectRoot 'tracks\under-investigation'
$staging = Join-Path $PSScriptRoot '.import-staging\under-investigation'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Under Investigation.jpg'
$sourceZipUrl = 'https://www.dropbox.com/scl/fi/co2kelrtgsu26sbv66rmb/Under-investigation.zip?rlkey=c2egqwrzjsihx74xmy2nzqne5&dl=1'
$loopWavPattern = '(?i)under investigation.*long loop.*0_54'
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  if (-not $found) { throw 'Missing Under investigation Long Loop 0_54.wav in extracted zip' }
...
function Resolve-Cover([string]$OutPath) {
  ...
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?search=Under+investigation&per_page=10" -TimeoutSec 60
  $match = @($wc | Where-Object { $_.name -match '(?i)under investigation' } | Select-Object -First 1)
...
# Metadata from track-meta.json (Tracks-Upload-Status-2013.xlsx export)
$description = 'A suspenseful and intriguing tune with a pinch of humor. Featuring Upright piano and Fender Rhodes'
$metaTags = @(
  'sneaky', 'spy', 'funny', 'careful', 'delicate', 'children', 'comedy', 'suspense', 'drama', 'humor',
  'cautious', 'investigation', 'fun', 'spooky', 'sophisticated', 'complicated', 'dramatic', 'suspenseful',
  'mysterious', 'crime'
)
$tags = @($metaTags | ForEach-Object { (Get-Culture).TextInfo.ToTitleCase($_.ToLowerInvariant()) })
$genre = 'Drama'
$bpm = 70
$instruments = 'piano, drums, Rhodes, strings'
$category2 = 'Comedy'
