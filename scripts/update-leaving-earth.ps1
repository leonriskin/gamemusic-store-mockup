# Import / refresh Leaving Earth demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$trackDir = Join-Path $ProjectRoot 'tracks\leaving-earth'
$staging = Join-Path $PSScriptRoot '.import-staging\leaving-earth'
$localFullZip = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV\Leaving Earth.zip'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Leaving Earth.jpg'
$packUrl = 'https://www.dropbox.com/scl/fi/e16ncddrqc7m9k3xmoryy/Leaving-Earth.zip?rlkey=v6lzlxxzt3pch96ak483n9azp&dl=1'
$loopWavName = 'Leaving Earth - short loop.wav'
$title = 'Leaving Earth'
$slug = 'leaving-earth'
$trackId = 24
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -Filter '*.wav' -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      ($_.BaseName -match '(?i)leaving earth.*\bshort loop\b')
    } |
...
function Resolve-Cover([string]$OutPath) {
  if (Test-Path -LiteralPath $coverFinal) {
    ...
    return 'Dropbox Final\Leaving Earth.jpg'
  }
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?slug=leaving-earth&per_page=1" -TimeoutSec 60
...
$description = 'A melancholic synthwave music with a strong beat and a futuristic vibe to it. Featuring electric guitars and synths'
$tags = @(
  'Menu', 'Level', 'Gameplay', 'Cyberpunk', 'Badass', 'Dark', 'Ominous', 'Dangerous',
  'Mystery', 'Bad', 'Scary', 'Horror', 'Futuristic', 'Scifi', 'Science', 'Apocalyptic',
  'Action', 'Beat', 'Serious', 'Underscore', 'Future', 'Retro', 'Electronic', 'Techno',
