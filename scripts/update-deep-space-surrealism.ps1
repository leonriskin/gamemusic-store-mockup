# Refresh Deep Space Surrealism loop.zip (full.wav only) and 128 kbps demo MP3 from Dropbox pack.
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$localBase = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV'
$srcZip = Join-Path $localBase 'Deep Space Surrealizm.zip'
$fullWavName = 'Deep Space Surrealizm full.wav'
$title = 'Deep Space Surrealism'
$slug = 'deep-space-surrealism'
...
function Resolve-FullWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      $_.BaseName -ieq 'Deep Space Surrealizm full'
    } |
...
$catalog.tracks = @($catalog.tracks | ForEach-Object {
  if ($_.title -eq $title -or $_.id -eq 17) {
    $updated = [ordered]@{}
