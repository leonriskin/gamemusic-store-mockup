# Import / refresh Sunny Island Pirates demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
#
...
#   Chosen loop: "Sunny Island Pirates Long Loop.wav" (case-insensitive match).
...
$trackDir = Join-Path $ProjectRoot 'tracks\sunny-island-pirates'
$staging = Join-Path $PSScriptRoot '.import-staging\sunny-island-pirates'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Sunny Island Pirates.jpg'
$sourceZipUrl = 'https://www.dropbox.com/scl/fi/oaz1sw6anxvhucvd8kgu8/Sunny-Island-Pirates.zip?rlkey=mnkyet5hn8jetqvnniultl3mb&dl=1'
$loopWavName = 'Sunny Island Pirates Long Loop.wav'
$loopWavPattern = '(?i)sunny island pirates.*long loop'
$title = 'Sunny Island Pirates'
$slug = 'sunny-island-pirates'
$trackId = 36
...
function Resolve-Cover([string]$OutPath) {
  ...
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?search=Sunny+Island+Pirates&per_page=10" -TimeoutSec 60
  $match = @($wc | Where-Object { $_.name -match '(?i)sunny island pirates' } | Select-Object -First 1)
...
$description = 'This track is pure tropical bliss. A real audio adventure full of fun, storytelling and ethnic island spirit. It has motifs from pirate and adventure songs packed in a colorful and an uplifting theme. It feels like a glimpse into a picturesque island full of coconut trees, perfect beaches, and lush green forests. Featuring a marimba, a steel drum, various percussion, piccolo flute, an accordion and a harpsichord. Suitable for games, animations, and travel videos.'
$metaTags = @(
  'beach', 'sunshine', 'sunny', 'tropical', 'animation', 'adventurous', 'ethnic', 'cheerful',
  'childish', 'cool', 'fun', 'funny', 'game', 'happy', 'humorous', 'jolly', 'joyful', 'jumpy',
  'light', 'lively', 'playful', 'positive', 'quirky', 'uplifting', 'jungle', 'forest', 'island',
  'pirates', 'fantasy', 'ship', 'sea', 'comic', 'paradise', 'caribbean', 'cuban', 'latin',
  'sailing', 'travel', 'vacation', 'video game'
)
$tags = @($metaTags | ForEach-Object { (Get-Culture).TextInfo.ToTitleCase($_.ToLowerInvariant()) })
$genre = 'Children'
$bpm = 180
$instruments = 'marimba, steel drum, percussion, piccolo flute, accordion, harpsichord'
$category2 = 'children'
