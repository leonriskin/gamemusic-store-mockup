# Refresh A Song For Naomi loop.zip (Loop1.wav only) and 128 kbps demo MP3 from Dropbox pack.
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$localBase = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV'
$srcZip = Join-Path $localBase 'A Song For Naomi.zip'
$loopWavName = 'A Song For Naomi - Loop1.wav'
$title = 'A Song For Naomi'
$slug = 'a-song-for-naomi'
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      $_.BaseName -ieq 'A Song For Naomi - Loop1'
    } |
...
function Expand-SourceZip([string]$ZipPath, [string]$Destination) {
  if (Test-Path $Destination) { Remove-Item $Destination -Recurse -Force }
  New-Item -ItemType Directory -Force -Path $Destination | Out-Null

  Expand-Archive -LiteralPath $ZipPath -DestinationPath $Destination -Force -ErrorAction SilentlyContinue
  $hasAudio = @(Get-ChildItem $Destination -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object { $_.Extension -match '(?i)\.(wav|aiff?|flac)$' }).Count -gt 0
  if ($hasAudio) { return }

  Remove-Item $Destination -Recurse -Force
  New-Item -ItemType Directory -Force -Path $Destination | Out-Null
  & tar -xf $ZipPath -C $Destination
  if ($LASTEXITCODE -ne 0) { throw "Failed to extract $ZipPath" }
}

if (-not (Test-Path -LiteralPath $srcZip)) { throw "Missing source zip: $srcZip" }

Expand-SourceZip $srcZip $staging
...
$catalog.tracks = @($catalog.tracks | ForEach-Object {
  if ($_.title -eq $title -or $_.id -eq 20) {
    $updated = [ordered]@{}
