# Import / refresh Zombie Apocalypse demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
#
...
#   Chosen loop: "Zombie Apocalypse Long Loop.wav" (case-insensitive match).
...
$trackDir = Join-Path $ProjectRoot 'tracks\zombie-apocalypse'
$staging = Join-Path $PSScriptRoot '.import-staging\zombie-apocalypse'
$localBase = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV'
$localFullZip = Join-Path $localBase 'Zombie Apocalypse.zip'
$localLoopWav = Join-Path $localBase 'Zombie Apocalypse Long Loop.wav'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Zombie Apocalypse.jpg'
$loopWavName = 'Zombie Apocalypse Long Loop.wav'
$loopWavPattern = '(?i)zombie apocalypse.*long loop'
...
function Resolve-Cover([string]$OutPath) {
  ...
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?search=Zombie+Apocalypse&per_page=10" -TimeoutSec 60
  $match = @($wc | Where-Object { $_.name -match '(?i)zombie apocalypse' } | Select-Object -First 1)
...
$description = 'A fast-paced techno-styled horror musical piece. Featuring heavy drum beats and massive impacts with piano chords and a dark ambient lead synth. Suitable for thrillers, horror movies, and action scenes. Can be used in animations, films, and video games.'
$metaTags = @(
  'horror', 'apocalypse', 'cinematic', 'disaster', 'action', 'dramatic', 'fast', 'fear', 'epic',
  'escape', 'fight', 'futuristic', 'zombie', 'war', 'scary', 'techno', 'danger', 'dark', 'evil',
  'suspense', 'screech', 'halloween', 'creepy', 'intense', 'aggressive', 'apocalyptic', 'death',
  'blood', 'chase'
)
$tags = @($metaTags | ForEach-Object { (Get-Culture).TextInfo.ToTitleCase($_.ToLowerInvariant()) })
$genre = 'Horror'
$bpm = 160
$instruments = 'drums, synth, piano'
