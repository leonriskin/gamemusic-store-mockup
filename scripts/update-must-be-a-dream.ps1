# Import / refresh Must Be a Dream demo.mp3 and loop.zip ($9 loop WAV only — no full pack).
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$trackDir = Join-Path $ProjectRoot 'tracks\must-be-a-dream'
$staging = Join-Path $PSScriptRoot '.import-staging\must-be-a-dream'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Must Be a Dream.jpg'
$coverFinalAlt = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Must Be A Dream.jpg'
$loopZipUrl = 'https://www.dropbox.com/scl/fi/ohtr309jjy9lqhtqyygej/Must-Be-a-Dream-loop.zip?rlkey=kosvtx8sarlm9linfd1cu9lxi&dl=1'
$loopWavName = 'Must Be a Dream - loop.wav'
$title = 'Must Be a Dream'
$slug = 'must-be-a-dream'
$trackId = 26
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -Filter '*.wav' -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      ($_.BaseName -match '(?i)must be a dream' -and $_.BaseName -notmatch 'short')
    } |
    Select-Object -First 1
  if (-not $found) { throw "Missing loop WAV in extracted zip" }
...
function Resolve-Cover([string]$OutPath) {
  ...
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?slug=must-be-a-dream&per_page=1" -TimeoutSec 60
...
& curl.exe -L --fail -o $loopStagingZip $loopZipUrl
if (-not (Test-Path $loopStagingZip)) { throw 'Could not obtain loop zip.' }
