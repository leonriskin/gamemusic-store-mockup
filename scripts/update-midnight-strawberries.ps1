# Refresh Midnight Strawberries loop.zip from Dropbox and 128 kbps demo MP3 from full loop WAV.
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$localBase = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV'
$srcZip = Join-Path $localBase 'Midnight Strawberries - full loop.zip'
$loopWavName = 'Midnight Strawberries - full loop.wav'
$title = 'Midnight Strawberries'
$slug = 'midnight-strawberries'
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      $_.BaseName -ieq 'Midnight Strawberries - full loop'
    } |
...
$catalog.tracks = @($catalog.tracks | ForEach-Object {
  if ($_.title -eq $title -or $_.id -eq 22) {
    $updated = [ordered]@{}
