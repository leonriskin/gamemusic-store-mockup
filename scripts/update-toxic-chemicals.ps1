# Import / refresh Toxic Chemicals demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
#
...
#   Chosen loop: "toxic chemicals long loop 01_26.wav" (case-insensitive match).
...
$trackDir = Join-Path $ProjectRoot 'tracks\toxic-chemicals'
$staging = Join-Path $PSScriptRoot '.import-staging\toxic-chemicals'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\Toxic Chemicals.jpg'
$sourceZipUrl = 'https://www.dropbox.com/scl/fi/bfzuka754qd1nlnnakzcv/Toxic-chemicals.zip?rlkey=xn3wwqe8tvld8d7hbfsv87d1j&dl=1'
$loopWavName = 'toxic chemicals long loop 01_26.wav'
$loopWavPattern = '(?i)toxic chemicals.*long loop.*01_26'
$title = 'Toxic Chemicals'
$slug = 'toxic-chemicals'
$trackId = 31
...
function Resolve-Cover([string]$OutPath) {
  ...
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?search=Toxic+Chemicals&per_page=10" -TimeoutSec 60
  $match = @($wc | Where-Object { $_.name -match '(?i)toxic chemicals' } | Select-Object -First 1)
...
$description = 'A cool downtempo electro-grime tune. Big beat with fat and dirty bass theme and atmospheric effects. Suitable for crime scenes'
$metaTags = @(
  'dirty', 'filthy', 'cool', 'downtempo', 'big beat', 'smooth', 'crime', 'gangsters', 'modern',
  'house', 'gun', 'criminals', 'police', 'electronic', 'wicked', 'vulgar', 'tough', 'street-smart',
  'strange', 'sinister', 'scary', 'raunchy', 'pushy', 'psychotic', 'powerful', 'ominous',
  'mysterious', 'mischievous', 'massive', 'intricate'
)
$tags = @($metaTags | ForEach-Object { (Get-Culture).TextInfo.ToTitleCase($_.ToLowerInvariant()) })
$genre = 'Electronic'
$bpm = 100
$instruments = 'beat, bass, synths, pads'
$category2 = 'Grime'
