# Refresh Burning Tracks loop.zip (WAV full only) and 128 kbps demo MP3 from Dropbox pack.
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$localBase = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV'
$srcZip = Join-Path $localBase 'Burning Tracks.zip'
$fullWavName = 'Burning Tracks - WAV full.wav'
...
function Resolve-FullWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      $_.BaseName -ieq 'Burning Tracks - WAV full'
    } |
...
$catalog.tracks = @($catalog.tracks | ForEach-Object {
  if ($_.title -eq $title -or $_.id -eq 19) {
    $updated = [ordered]@{}
