# Import / refresh No Way Out demo.mp3 and loop.zip ($9 loop WAV only — no full pack).
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$trackDir = Join-Path $ProjectRoot 'tracks\no-way-out'
$staging = Join-Path $PSScriptRoot '.import-staging\no-way-out'
$coverFinal = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops IMG\Final\No Way Out.jpg'
$loopZipUrl = 'https://www.dropbox.com/scl/fi/nd2ved7s6m8ln6r1eiwe5/No-Way-Out-loop.zip?rlkey=fzw5m6nl0vmbyhplh3zfln2df&dl=1'
$loopWavName = 'No Way Out - loop.wav'
$title = 'No Way Out'
$slug = 'no-way-out'
$trackId = 25
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -Filter '*.wav' -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      ($_.BaseName -match '(?i)no way out.*\bloop\b' -and $_.BaseName -notmatch 'short')
    } |
...
function Resolve-Cover([string]$OutPath) {
  if (Test-Path -LiteralPath $coverFinal) {
    ...
    return 'Dropbox Final\No Way Out.jpg'
  }
  $wc = Invoke-RestMethod "https://gamemusicandsoundeffects.com/wp-json/wc/store/products?slug=no-way-out&per_page=1" -TimeoutSec 60
...
$description = 'A futuristic electronic track with a cyberpunk vibe. Rhythmic and melancholic with a suspenseful vibe'
$tags = @(
