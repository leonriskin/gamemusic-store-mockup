# Refresh Palm Tree Getaway Island demo.mp3, loop.zip ($9 loop WAV), and pack.zip ($12 full pack).
$ErrorActionPreference = 'Stop'
$ProjectRoot = Join-Path $PSScriptRoot '..'
$trackDir = Join-Path $ProjectRoot 'tracks\palm-tree-getaway-island'
$staging = Join-Path $PSScriptRoot '.import-staging\palm-tree-getaway-island'
$localFullZip = 'C:\Dropbox\Dropbox\1. My Documents\LEON\Audio Temp Ex\Leon Riskin Music\WordPress\Music Loops WAV\Palm Tree Getaway Island.zip'
$packUrl = 'https://www.dropbox.com/scl/fi/f2k74o46ij0rh7ervwm53/Palm-Tree-Getaway-Island.zip?rlkey=nb2bn93pgvw886ryse9fa0r8l&dl=1'
$loopWavName = 'Palm Tree Getaway Island - loop.wav'
$title = 'Palm Tree Getaway Island'
$slug = 'palm-tree-getaway-island'
...
function Resolve-LoopWav([string]$ExtractRoot) {
  ...
  $found = Get-ChildItem $ExtractRoot -Recurse -Filter '*.wav' -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -ieq $loopWavName -or $_.BaseName -match '(?i)palm tree getaway island.*\bloop\b' -and $_.BaseName -notmatch 'short|no sfx' } |
...
$catalog.tracks = @($catalog.tracks | ForEach-Object {
  if ($_.title -eq $title -or $_.id -eq 7) {
    $updated = [ordered]@{}
...
Write-Host "Updated tracks/$slug/loop.zip (contains $loopWavName)"
